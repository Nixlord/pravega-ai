...
def line_transform(crop_img):
    ...
    if len(linesP_img) is not None:
        ...
        min_length = math.sqrt(
            abs((linesP_img[0][0][2] - linesP_img[0][0][0]) ** 2 + (linesP_img[0][0][3] - linesP_img[0][0][1]) ** 2))
        hour_hand = []
    ...
    if len(linesP_img_final) == 2 and len(linesP_img_final[0]) > 0 and len(linesP_img_final[1]) > 0:
        for i in range(0, len(linesP_img_final)):
            ...
            if (abs((cdst.shape[0] / 2) - l3[0]) < abs((cdst.shape[0] / 2) - l3[2])) or (
                    abs((cdst.shape[1] / 2) - l3[1]) < abs((cdst.shape[1] / 2) - l3[3])):
                angle = -angle1
            else:
                if dy >= 0:
                    angle = 180 - angle1
...
def run(filename):
    crop_img = hough_transform(filename)
    angles = line_transform(crop_img)
    getTime(angles)
